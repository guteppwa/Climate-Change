...
class Building2:
    ...
    def enter_input(self):
        ...
        w1 = Scale(Root,label ="Latitude-Begin", from_=90, to=-90,variable = var_lat2)
        w1.pack()
        w1.place(relx=0.83, rely=0.35, anchor="c")
        var_lat1 = DoubleVar(Root)
        w1 = Scale(Root,label ="Latitude-End", from_=90, to=-90,variable = var_lat1)
        w1.pack()
        w1.place(relx=0.93, rely=0.35, anchor="c")
        var_lon1 = DoubleVar(Root)
        w3 = Scale(Root,label ="Longtitude-Begin", from_=-180, to=180 , orient=HORIZONTAL,variable = var_lon1)
        w3.pack()
        w3.place(relx=0.83, rely=0.2, anchor="c")
        var_lon2 = DoubleVar(Root)
        w3 = Scale(Root,label ="Longtitude-End", from_=-180, to=180 , orient=HORIZONTAL,variable = var_lon2)
        w3.pack()
        w3.place(relx=0.93, rely=0.2, anchor="c")
        
        #year selection
        w = Label(Root, text="Select Year")
        w.pack()
        w.place(relx=0.82, rely=0.45, anchor="c")
        ...
        option.pack()
        option.place(relx=0.9, rely=0.45, anchor="c")
        ...
        #send data
        button1 = Button(Root, text="Send Data for Pic", command=lambda: obj.plot_file(var_year.get(),var_mon.get(),var_day.get(),var_path.get(),var_lat1.get(),var_lat2.get(),var_lon1.get(),var_lon2.get(),var_mea.get(),path_img1))
        button1.pack()
        button1.place(relx=0.87, rely=0.65, anchor="c")
    
        
    
    
    def plot_file(self,year,month,day,path,lats_s,lats_e,lons_s,lons_e,var,path_img):
        #nc file name 
        my_filename = (path)
    ...
    def open_img(self):
        def Click1():
            ...
            c_image1.pack()
            c_image1.place(relx=0.87, rely=0.75, anchor="c")
        buttonImg1 = Button(Root, text="Open Pic", command = Click1)
        buttonImg1.pack()
        buttonImg1.place(relx=0.87, rely=0.7, anchor="c")
